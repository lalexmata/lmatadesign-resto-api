...
export class PaymentService {
    ...
    async createPayment(dto: CreatePaymentDto) {
        ...
        const method = await this.paymentMethodRepo.findOne({ where: { id: dto.method_id } });
        if (!method) {
            throw new NotFoundException(`Método de pago con ID ${dto.method_id} no encontrado`);
        }

        const totalPagado = order.payments?.reduce((acc, pago) => acc + Number(pago.amount), 0) || 0;
        const pendiente = Number(order.total) - totalPagado;

        if (dto.amount > pendiente) {
            throw new BadRequestException(`El monto excede el total pendiente de $${pendiente}`);
        }
    ...
    async getPaymentsByOrder(orderId: number) {
        ...
        const totalPaid = order.payments.reduce((acc, pago) => acc + Number(pago.amount), 0);
