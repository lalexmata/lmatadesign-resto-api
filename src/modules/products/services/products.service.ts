...
export class ProductsService {
  constructor(
    ...
    private productsInventoryRepository: Repository<ProductInventory>,
  ) {}
    ...
    for (const ingredient of data.ingredients) {
      ...
      if (!inventory)
        throw new NotFoundException(
          `Insumo ID ${ingredient.inventory_id} no encontrado`,
        );
  ...
  async update(id: number, data: UpdateProductDto) {
    ...
    if (data.ingredients) {
      await this.productsInventoryRepository.delete({
